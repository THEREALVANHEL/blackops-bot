...
class EnhancedPetSystem(commands.Cog):
    ...
    @app_commands.describe(parent1="Name of first parent pet", parent2="Name of second parent pet")
    async def breed_pets(self, interaction: discord.Interaction, parent1: str, parent2: str):
        user_data = database.db.get_user_data(interaction.user.id)
        pets = user_data.get("pets", [])
        
        # Find parent pets
        pet1 = next((p for p in pets if p["name"].lower() == parent1.lower()), None)
        pet2 = next((p for p in pets if p["name"].lower() == parent2.lower()), None)
        
        if not pet1:
            await interaction.response.send_message(f"❌ You don't have a pet named '{parent1}'.", ephemeral=True)
            return
        
        if not pet2:
            await interaction.response.send_message(f"❌ You don't have a pet named '{parent2}'.", ephemeral=True)
            return
        
        if pet1.get("pet_id") == pet2.get("pet_id"):
            await interaction.response.send_message("❌ A pet cannot breed with itself!", ephemeral=True)
            return
        
        # Check breeding requirements
        if pet1.get("level", 1) < 10 or pet2.get("level", 1) < 10:
            await interaction.response.send_message("❌ Both pets must be at least level 10 to breed!", ephemeral=True)
            return
        
        if pet1.get("happiness", 50) < 70 or pet2.get("happiness", 50) < 70:
            await interaction.response.send_message("❌ Both pets must be at least 70% happy to breed!", ephemeral=True)
            return
        
        # Check breeding cooldown
        breeding_cooldown = 86400 * 7  # 1 week
        last_bred1 = pet1.get("last_bred", 0)
        last_bred2 = pet2.get("last_bred", 0)
        
        if time.time() - last_bred1 < breeding_cooldown:
            next_time = last_bred1 + breeding_cooldown
            await interaction.response.send_message(f"❌ {pet1['name']} needs to wait until <t:{int(next_time)}:R> before breeding again.", ephemeral=True)
            return
        
        if time.time() - last_bred2 < breeding_cooldown:
            next_time = last_bred2 + breeding_cooldown
            await interaction.response.send_message(f"❌ {pet2['name']} needs to wait until <t:{int(next_time)}:R> before breeding again.", ephemeral=True)
            return
        
        # Breeding cost
        breeding_cost = 10000
        if user_data.get("coins", 0) < breeding_cost:
            needed = breeding_cost - user_data.get("coins", 0)
            await interaction.response.send_message(f"❌ Breeding costs {breeding_cost:,} coins. You need {needed:,} more coins.", ephemeral=True)
            return
        
        # Check pet limit
        max_pets = 5 + (user_data.get("level", 1) // 10)
        if len(pets) >= max_pets:
            await interaction.response.send_message(f"❌ You have reached your pet limit ({max_pets}). Release or evolve some pets first.", ephemeral=True)
            return
        
        await interaction.response.defer()
        
        # Perform breeding
        database.db.remove_coins(interaction.user.id, breeding_cost)
        
        # Determine offspring characteristics
        # Species inheritance (favor higher rarity parent)
        rarity_weights = {"common": 1, "uncommon": 2, "rare": 4, "legendary": 8, "mythic": 16}
        
        if rarity_weights[pet1["rarity"]] > rarity_weights[pet2["rarity"]]:
            primary_parent = pet1
            secondary_parent = pet2
        else:
            primary_parent = pet2
            secondary_parent = pet1
        
        # 70% chance to inherit primary parent's species, 30% secondary
        if random.random() < 0.7:
            offspring_species = primary_parent["species"]
            offspring_rarity = primary_parent["rarity"]
        else:
            offspring_species = secondary_parent["species"]
            offspring_rarity = secondary_parent["rarity"]
        
        # Small chance for rarity upgrade
        if random.random() < 0.1:  # 10% chance
            rarity_list = ["common", "uncommon", "rare", "legendary", "mythic"]
            current_index = rarity_list.index(offspring_rarity)
            if current_index < len(rarity_list) - 1:
                offspring_rarity = rarity_list[current_index + 1]
        
        # Get offspring data
        offspring_data = PET_SPECIES[offspring_rarity][offspring_species]
        
        # Mix personalities
        personalities = [pet1.get("personality", "Gentle"), pet2.get("personality", "Gentle")]
        offspring_personality = random.choice(personalities + list(PET_PERSONALITIES.keys())[:2])  # Small chance for new personality
        
        # Calculate mixed stats (average of parents with some randomness)
        mixed_stats = {}
        base_stats = offspring_data["base_stats"]
        
        for stat in base_stats:
            parent1_stat = pet1["stats"].get(stat, base_stats[stat])
            parent2_stat = pet2["stats"].get(stat, base_stats[stat])
            
            # Average with some randomness and bias toward base stats
            mixed_value = int((parent1_stat + parent2_stat + base_stats[stat] * 2) / 4)
            mixed_value += random.randint(-5, 10)  # Slight randomness with positive bias
            mixed_stats[stat] = max(base_stats[stat] // 2, mixed_value)  # Minimum floor
        
        # Apply personality modifiers
        personality_mods = PET_PERSONALITIES[offspring_personality]
        for stat, multiplier in personality_mods.items():
            if stat in mixed_stats:
                mixed_stats[stat] = int(mixed_stats[stat] * multiplier)
        
        # Create offspring
        offspring = {
            "pet_id": f"{interaction.user.id}_{int(time.time())}_bred",
            "name": f"Baby {offspring_species}",
            "species": offspring_species,
            "rarity": offspring_rarity,
            "emoji": offspring_data["emoji"],
            "personality": offspring_personality,
            "level": 1,
            "experience": 0,
            "stats": mixed_stats,
            "current_hp": mixed_stats["hp"],
            "happiness": 80,  # Born happy
            "hunger": 30,
            "energy": 100,
            "last_fed": time.time(),
            "last_played": time.time(),
            ...
            "generation": max(pet1.get("generation", 1), pet2.get("generation", 1)) + 1
        }
        
        # Update parent breeding times
        for i, p in enumerate(pets):
            if p.get("pet_id") == pet1.get("pet_id") or p.get("pet_id") == pet2.get("pet_id"):
                pets[i]["last_bred"] = time.time()
        
        # Add offspring to pets
        pets.append(offspring)
        database.db.update_user_data(interaction.user.id, {"pets": pets})
        
        embed = discord.Embed(
            title="🥚➜🐾 Successful Breeding!",
            description=f"**{pet1['name']}** and **{pet2['name']}** have produced offspring!",
            color=discord.Color.green(),
            timestamp=datetime.utcnow()
        )
        
        embed.add_field(
            name="👶 Offspring Details",
            value=f"**Name:** {offspring['name']}\n**Species:** {offspring_species}\n**Rarity:** {offspring_rarity.title()}\n**Personality:** {offspring_personality}",
            inline=True
        )
        
        embed.add_field(
            name="🧬 Genetics",
            value=f"**Parents:** {pet1['name']} × {pet2['name']}\n**Generation:** {offspring['generation']}\n**Inherited:** Mixed traits",
            inline=True
        )
        
        # Show mixed stats
        stats_comparison = []
        for stat, value in mixed_stats.items():
            p1_stat = pet1["stats"].get(stat, 0)
            p2_stat = pet2["stats"].get(stat, 0)
            stats_comparison.append(f"**{stat.upper()}:** {value} ({p1_stat}+{p2_stat})")
        
        embed.add_field(name="📊 Mixed Stats", value="\n".join(stats_comparison), inline=False)
        embed.add_field(name="💰 Cost", value=f"{breeding_cost:,} coins", inline=True)
        embed.add_field(name="💡 Tip", value="Use `/pet` to rename and interact with your new pet!", inline=False)
        
        embed.set_thumbnail(url=interaction.user.display_avatar.url)
        embed.set_footer(text="Congratulations on your new family member!")
